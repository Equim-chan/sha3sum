...
func verifyChecksumLine(line string) int8 {
	...
	digest, err := computeFile(filename, uint(len(checksum)))
	if err != nil {
...
func runVerify(filelist []string) int {
	...
	for _, filename := range filelist {
		...
		unmatchErrCount += uint64(errStat[0])
		failReadErrCount += uint64(errStat[1])
		wrongFormatErrCount += uint64(errStat[2])
	}
